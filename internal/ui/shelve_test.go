...
func shelveRepoModel(t *testing.T, dir string) Model {
	...
	m := New(git.New(dir), samplePaths(), cfg, st, store.Prefs{})
	m.loading = false
	m.current = "feature"
	m.expanded["ABC-1"] = true
	m.cursor = 1
...
// which is what `git clone` checks out; leaving it to init.defaultBranch points
// HEAD at a branch we never push on any machine where that default is not main.
